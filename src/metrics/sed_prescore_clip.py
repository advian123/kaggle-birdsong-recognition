...
class SedPreScoreClip(Metric):
    def __init__(self, threshold=0.5, output_transform=lambda x: x):
        self._score = None
        self._num_items = None
        self.threshold = threshold
        super(SedPreScoreClip, self).__init__(output_transform=output_transform)

    @reinit__is_reduced
    def reset(self):
        self._score = 0.0
        self._num_items = 0
        super(SedPreScoreClip, self).reset()
        ...
        for i, (p, t) in enumerate(zip(pred,target)):
            if t.sum()==0.0:
                if ((p>=self.threshold).sum())>0.0:
                    score = precision_score([1], [0], average='micro')
                else:
                    score = precision_score([1], [1], average='micro')
            else:
                score = precision_score([t], [p>=self.threshold], average='micro')
            self._score += score
            self._num_items +=1
